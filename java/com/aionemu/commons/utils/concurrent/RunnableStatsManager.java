...
public final class RunnableStatsManager
{
	...
	private static final class ClassStat
	{
		...
		private MethodStat getMethodStat(String methodName, boolean synchronizedAlready)
		{
			...
			final MethodStat methodStat = new MethodStat(this.className, methodName);
			(this.methodNames = Arrays.copyOf(this.methodNames, this.methodNames.length + 1))[this.methodNames.length - 1] = methodName;
			this.methodStats = Arrays.copyOf(this.methodStats, this.methodStats.length + 1);

			return this.methodStats[this.methodNames.length - 1] = methodStat;
		}
	}
