...
public abstract class QuestHandler extends AbstractQuestHandler
{
	...
	public boolean playQuestMovie(QuestEnv env, int MovieId) {
		Player player = env.getPlayer();
		PacketSendUtility.sendPacket(player, new S_PLAY_CUTSCENE(0, MovieId));
		return false;
	}
	...
	public boolean useQuestObject(QuestEnv env, int step, int nextStep, boolean reward, int varNum, int addItemId, int addItemCount, int removeItemId, int removeItemCount, int movieId) {
		return useQuestObject(env, step, nextStep, reward, varNum, addItemId, addItemCount, removeItemId, removeItemCount, movieId, false);
	}
	...
	public boolean useQuestItem(final QuestEnv env, final Item item, final int step, final int nextStep,
		...
		if (qs.getQuestVarById(varNum) == step) {
			PacketSendUtility.broadcastPacket(player, new S_USE_ITEM(player.getObjectId(), objectId, itemId, 3000, 0, 0), true);
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					PacketSendUtility.broadcastPacket(player, new S_USE_ITEM(player.getObjectId(), objectId, itemId, 0, 1, 0), true);
	...
	private void sendDialogPacket(QuestEnv env, int dialogId) {
		...
		PacketSendUtility.sendPacket(env.getPlayer(), new S_NPC_HTML_MESSAGE(objId, dialogId, questId));
	}
	...
	private void sendQuestSelectionPacket(QuestEnv env, int dialogId) {
		...
		PacketSendUtility.sendPacket(env.getPlayer(), new S_NPC_HTML_MESSAGE(objId, dialogId));
	}
