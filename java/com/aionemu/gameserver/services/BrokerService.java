...
public class BrokerService {
    ...
    public void showRequestedItems(Player player, int clientMask, int sortType, int startPage, List<Integer> itemList) {
        ...
        PacketSendUtility.sendPacket(player, new S_VENDOR(searchItems, totalSearchItemsCount, startPage));
    }
    ...
    public void registerItem(Player player, int itemUniqueId, long count, long PricePerItem, boolean isSplitSell) {
        ...
        if (registeredItemsCount > 14) {
            PacketSendUtility.sendPacket(player, new S_VENDOR(BrokerMessages.NO_SPACE_AVAIABLE.getId()));
            return;
        ...
        if (player.getInventory().getKinah() < registrationCommition) {
            PacketSendUtility.sendPacket(player, new S_VENDOR(BrokerMessages.NO_ENOUGHT_KINAH.getId()));
            return;
        }
        ...
        PacketSendUtility.sendPacket(player, new S_VENDOR(newBrokerItem, 0, registeredItemsCount));
    }
    ...
    public void CalcItemAveLowHigh(Player player, int itemUniqueId) {
        ...
        PacketSendUtility.sendPacket(player, new S_VENDOR(itemUniqueId, Ave7day, CurrentLow, CurrentHigh, IsLowHighSame));
    }
    ...
    public void showAddItemWindow(Player player, int itemObjectId) {
        ...
        if (items.size() < 1) {
            PacketSendUtility.sendPacket(player, new S_VENDOR(itemObjectId, 1, 1, 1, true));
        } else {
            long[] avgMaxMin = getAvgMaxMinPrice(items);
            PacketSendUtility.sendPacket(player, new S_VENDOR(itemObjectId, avgMaxMin[0], avgMaxMin[1], avgMaxMin[2], avgMaxMin[1] == avgMaxMin[2]));
        }
    }
    ...
    public void showRegisteredItems(Player player) {
        ...
        PacketSendUtility.sendPacket(player, new S_VENDOR(registeredItems.toArray(new BrokerItem[registeredItems.size()])));
    }
    ...
    public void showSettledItems(Player player) {
        ...
        PacketSendUtility.sendPacket(player, new S_VENDOR(settledItems.toArray(new BrokerItem[settledItems.size()]), totalKinah));
    }
    ...
    public void settleAccount(Player player) {
        ...
        if (!itemsLeft) {
            PacketSendUtility.sendPacket(player, new S_VENDOR(false, 0));
        }
    }
    ...
    public void onPlayerLogin(Player player) {
        ...
        for (BrokerItem item : brokerSettledItems.values()) {
            if (item != null && playerId == item.getSellerId()) {
                PacketSendUtility.sendPacket(player, new S_VENDOR(true, getTotalSettledKinah(player)));
                break;
            }
        }
    }
