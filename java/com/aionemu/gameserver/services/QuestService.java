...
public final class QuestService
{
			...
			if (isLastRepeat && template.isUseSingleClassReward() || template.isUseRepeatedClassReward()) {
				...
				switch (playerClass) {
					case ASSASSIN: {
						...
						classRewardItem = getQuestItemsbyClass(id, template.getThundererSelectableReward(), selRewIndex);
						break;
					}
					default:
						break;
	...
	private static void giveReward(QuestEnv env, Rewards rewards) {
		...
		if (rewards.getGold() != null) {
			...
			player.getCommonData().setAdvencedStigmaSlotSize(player.getCommonData().getAdvencedStigmaSlotSize() + 1);
			PacketSendUtility.sendPacket(player, S_EVENT.stigmaSlots(player.getCommonData().getAdvencedStigmaSlotSize()));
		}
	...
	private static boolean setFinishingState(QuestEnv env, QuestTemplate template, int reward) {
		...
		PacketSendUtility.sendPacket(player, new S_QUEST(id, qs.getStatus(), qs.getQuestVars().getQuestVars()));
		player.getController().updateNearbyQuests();
	...
	public static void startMission(QuestEnv env, QuestStatus status) {
		...
		PacketSendUtility.sendPacket(player, new S_QUEST(questId, status.value(), 0));
	}
	...
	public static boolean startEventQuest(QuestEnv env, QuestStatus questStatus) {
		...
		int id = env.getQuestId();
		Player player = env.getPlayer();
		PacketSendUtility.sendPacket(player, new S_QUEST(id, questStatus, 0));
		if ((player.getLevel() < template.getMinlevelPermitted()) && (template.getMinlevelPermitted() != 999)) {
			return false;
		} if (template.getMaxlevelPermitted() != 0 && player.getLevel() > template.getMaxlevelPermitted()) {
			return false;
				...
				if (drop.isDropEachMemberGroup()) {
					...
					for (Player p : pls) {
						...
						if (player.getPlayerGroup2().getLootGroupRules().getLootRule() != LootRuleType.FREEFORALL) {
							PacketSendUtility.sendPacket(p, new S_LOOT(npc.getObjectId(), 0));
						}
					}
					pls.clear();
				}
	...
	private static DropItem regQuestDropItem(QuestDrop drop, int index, Integer winner) {
		DropItem item = new DropItem(drop.getItemId());
	...
	public static boolean questTimerStart(QuestEnv env, int timeInSeconds) {
		...
		PacketSendUtility.sendPacket(player, new S_QUEST(env.getQuestId(), timeInSeconds));
		return true;
	}
	...
	public static boolean questTimerEnd(QuestEnv env) {
		...
		PacketSendUtility.sendPacket(player, new S_QUEST(env.getQuestId(), 0));
		return true;
	}
	...
	public static boolean abandonQuest(Player player, int questId) {
		...
		PacketSendUtility.sendPacket(player, new S_QUEST(questId));
		player.getController().updateZone();
		player.getController().updateNearbyQuests();
		return true;
	}
