...
import com.aionemu.gameserver.network.aion.serverpackets.S_PERSONAL_SHOP;
...
public class PrivateStoreService {
  ...
  private static final Logger log = LoggerFactory.getLogger("EXCHANGE_LOG");
	...
	private static void createStore(Player activePlayer) {
		...
		PacketSendUtility.broadcastPacket(activePlayer, new S_ACTION(activePlayer, EmotionType.OPEN_PRIVATESHOP, 0, 0),
			true);
	}
	...
	public static void closePrivateStore(Player activePlayer) {
		...
		PacketSendUtility.broadcastPacket(activePlayer, new S_ACTION(activePlayer, EmotionType.CLOSE_PRIVATESHOP, 0, 0),
			true);
	}
	...
	public static void openPrivateStore(Player activePlayer, String name) {
        ...
        if (name != null) {
            ...
            PacketSendUtility.broadcastPacket(playerActive, new S_PERSONAL_SHOP(playerActive.getObjectId(), name), true);
        } else {
            PacketSendUtility.broadcastPacket(playerActive, new S_PERSONAL_SHOP(playerActive.getObjectId(), ""), true);
        }
    }
}