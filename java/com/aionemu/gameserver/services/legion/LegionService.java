...
public class LegionService {
	 ...
	 *
	 * @param legionMember LegionMember
	 */
	 ...
	 * This method will add a new legion member to the cache
	 *
	 * @param legionMemberEx LegionMemberEx
	 */
	 ...
	 *
	 * @param legionMember LegionMemberEx
	 */
	...
	public Legion getLegion(String legionName) {
		 ...
		 */
		Legion legion = LegionDAO.loadLegion(legionName);
	...
	public Legion getLegion(int legionId) {
		 ...
		 * Else load the legion information from the database
		 */
		Legion legion = LegionDAO.loadLegion(legionId);
	...
	public void requestDisbandLegion(Creature npc, final Player activePlayer) {
		...
		if (legionRestrictions.canDisbandLegion(activePlayer, legion)) {
			...
			boolean disbandResult = activePlayer.getResponseRequester().putRequest(S_ASK.STR_GUILD_DISPERSE_STAYMODE,
							disbandResponseHandler);
			if (disbandResult) {
				PacketSendUtility.sendPacket(activePlayer, new S_ASK(S_ASK.STR_GUILD_DISPERSE_STAYMODE, 0, 0));
			}
		}
	}
	...
	public void createLegion(Player activePlayer, String legionName) {
		if (legionRestrictions.canCreateLegion(activePlayer, legionName)) {
			...
			activePlayer.getInventory().decreaseKinah(LegionLevel.LEVEL1.getKinah());
			...
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x05, (int) (System.currentTimeMillis() / 1000), ""));
			/**
			...
			BattlePassService.getInstance().onUpdateBattlePassMission(activePlayer, 0, 1, BattlePassAction.JOIN_LEGION);

			/**
			 * Send required packets
			 */
			PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CREATED(legion.getLegionName()));
		}
	}
			...
			RequestResponseHandler responseHandler = new RequestResponseHandler(activePlayer) {
				...
				public void acceptRequest(Creature requester, Player responder) {
					if (!targetPlayer.getCommonData().isOnline()) {
						PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_NO_SUCH_USER(targetPlayer.getName()));
					} else {
						...
						if (legion.addLegionMember(playerObjId)) {
							...
							BattlePassService.getInstance().onUpdateBattlePassMission(targetPlayer, 0, 1, BattlePassAction.JOIN_LEGION);
						} else {
							PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_CAN_NOT_ADD_MEMBER_ANY_MORE);
				...
				@Override
				public void denyRequest(Creature requester, Player responder) {
					PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_HE_REJECTED_INVITATION(targetPlayer.getName()));
				}
			};
			boolean requested = targetPlayer.getResponseRequester().putRequest(S_ASK.STR_GUILD_INVITE_I_JOINED_MSGBOX, responseHandler);
			if (!requested) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_OTHER_IS_BUSY);
			} else {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_SENT_INVITE_MSG_TO_HIM(targetPlayer.getName()));
				PacketSendUtility.sendPacket(targetPlayer, new S_ASK(S_ASK.STR_GUILD_INVITE_I_JOINED_MSGBOX, 0, 0, legion.getLegionName(), legion.getLegionLevel() + "", activePlayer.getName()));
			}
		}
	}
	...
	private void displayLegionMessage(Player targetPlayer, Entry<Timestamp, String> currentAnnouncement) {
		if (currentAnnouncement != null) {
			PacketSendUtility.sendPacket(targetPlayer, S_MESSAGE_CODE.STR_GUILD_NOTICE(currentAnnouncement.getValue(), (int) (currentAnnouncement.getKey().getTime() / 1000)));
		}
	}
	...
	private void appointBrigadeGeneral(final Player activePlayer, final Player targetPlayer) {
		if (legionRestrictions.canAppointBrigadeGeneral(activePlayer, targetPlayer)) {
			...
			RequestResponseHandler responseHandler = new RequestResponseHandler(activePlayer) {
				...
				public void acceptRequest(Creature requester, Player responder) {
					if (!targetPlayer.getCommonData().isOnline()) {
						PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_MASTER_NO_SUCH_USER);
					} else {
						...
						if (legionMember.getRank().getRankId() > LegionRank.BRIGADE_GENERAL.getRankId()) {
							...
							PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_MEMBER_INFO(activePlayer, 0, ""));
							// Promote member to Brigade General
							legionMember.setRank(LegionRank.BRIGADE_GENERAL);
							PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_MEMBER_INFO(targetPlayer, 1300273, targetPlayer.getName()));
							PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x08));
				...
				@Override
				public void denyRequest(Creature requester, Player responder) {
					PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_MASTER_HE_DECLINE_YOUR_OFFER(targetPlayer.getName()));
				}
			};
			boolean requested = targetPlayer.getResponseRequester().putRequest(S_ASK.STR_GUILD_CHANGE_MASTER_DO_YOU_ACCEPT_OFFER, responseHandler);
			if (!requested) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_MASTER_SENT_CANT_OFFER_WHEN_HE_IS_QUESTION_ASKED);
			} else {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_MASTER_SENT_OFFER_MSG_TO_HIM(targetPlayer.getName()));
				PacketSendUtility.sendPacket(targetPlayer, new S_ASK(S_ASK.STR_GUILD_CHANGE_MASTER_DO_YOU_ACCEPT_OFFER, activePlayer.getObjectId(), 0, activePlayer.getName()));
			}
		}
	}
	...
	private void appointRank(Player activePlayer, String charName, int rankId) {
		...
		if (LM == null) {
			...
			LegionMemberDAO.storeLegionMember(legionMember.getObjectId(), legionMember);
			LM.setRank(rank);
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_MEMBER_INFO(LM, msgId, LM.getName()));
		}
	}
	...
	private void appointRank(Player activePlayer, Player targetPlayer, int rankId) {
		if (legionRestrictions.canAppointRank(activePlayer, targetPlayer.getObjectId())) {
			...
			legionMember.setRank(rank);
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_MEMBER_INFO(targetPlayer, msgId, targetPlayer.getName()));
		}
	}
	...
	private void changeSelfIntro(Player activePlayer, String newSelfIntro) {
		if (legionRestrictions.canChangeSelfIntro(activePlayer, newSelfIntro)) {
			...
			PacketSendUtility.broadcastPacketToLegion(legionMember.getLegion(), new S_CHANGE_GUILD_MEMBER_INTRO(activePlayer.getObjectId(), newSelfIntro));
			PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_WRITE_INTRO_DONE);
		}
	}
	...
	public void changePermissions(Legion legion, short deputyPermission, short centurionPermission, short legionarPermission, short volunteerPermission) {
		if (legion.setLegionPermissions(deputyPermission, centurionPermission, legionarPermission, volunteerPermission)) {
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x02, legion));
		}
	}
	...
	public void changeLevel(Legion legion, int newLevel, boolean save) {
		...
		PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x00, legion));
		PacketSendUtility.broadcastPacketToLegion(legion, S_MESSAGE_CODE.STR_GUILD_EVENT_LEVELUP(newLevel));
		if (save) {
			storeLegion(legion);
		}
	}
	...
	private void changeNickname(Player activePlayer, String charName, String newNickname) {
		...
		if ((targetPlayer = World.getInstance().findPlayer(charName)) != null) {
			...
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_MEMBER_NICKNAME(legionMember.getObjectId(), newNickname));
	...
	private void updateAfterDisbandLegion(Legion legion) {
		for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
			PacketSendUtility.broadcastPacket(onlineLegionMember, new S_CHANGE_GUILD_OTHER(onlineLegionMember.getObjectId(), 0, "", 0), true);
			PacketSendUtility.sendPacket(onlineLegionMember, new S_DELETE_GUILD_MEMBER(1300302, 0, legion.getLegionName()));
	...
	private void updateMembersEmblem(Legion legion, LegionEmblemType emblemType) {
		...
		for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
			PacketSendUtility.broadcastPacket(onlineLegionMember, new S_GUILD_EMBLEM_UPDATED(legion.getLegionId(), legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b(), emblemType), true);
	...
	private void updateMembersOfDisbandLegion(Legion legion, int unixTime) {
		for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
			PacketSendUtility.sendPacket(onlineLegionMember, new S_CHANGE_GUILD_MEMBER_INFO(onlineLegionMember, 1300303, unixTime + ""));
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x06, unixTime));
		}
	}
	...
	private void updateMembersOfRecreateLegion(Legion legion) {
		for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
			PacketSendUtility.sendPacket(onlineLegionMember, new S_CHANGE_GUILD_MEMBER_INFO(onlineLegionMember, 1300307, ""));
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x07));
		}
	}
	...
	public void storeLegionEmblem(Player activePlayer, LegionEmblem customEmblem) {
		...
		PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_EMBLEM);
	}
	...
	public void openLegionWarehouse(Player player, Npc npc) {
		if (legionRestrictions.canOpenWarehouse(player)) {
			...
			PacketSendUtility.sendPacket(player, new S_CHANGE_GUILD_INFO(0x04, player.getLegion()));//kinah
			...
			if (!isEmpty) {
				...
				while (!splitter.isLast()) {
					PacketSendUtility.sendPacket(player, new S_LOAD_WAREHOUSE(splitter.getNext(), storageId, whLvl, splitter.isFirst(), player));
				}
			}
			PacketSendUtility.sendPacket(player, new S_LOAD_WAREHOUSE(null, storageId, whLvl, isEmpty, player));
			PacketSendUtility.sendPacket(player, new S_NPC_HTML_MESSAGE(npc.getObjectId(), 25));
		}
	}
	...
	public void recreateLegion(Npc npc, Player activePlayer) {
		...
		if (legionRestrictions.canRecreateLegion(activePlayer, legion)) {
			...
			boolean disbandResult = activePlayer.getResponseRequester().putRequest(S_ASK.STR_GUILD_DISPERSE_STAYMODE_CANCEL, disbandResponseHandler);
			if (disbandResult) {
				PacketSendUtility.sendPacket(activePlayer, new S_ASK(S_ASK.STR_GUILD_DISPERSE_STAYMODE_CANCEL, 0, 0));
			}
		}
	}
	...
	public void performRankingUpdate(Map<Integer, Integer> legionRanking) {
		...
		while (legionsIterator.hasNext()) {
			...
			if (legionRanking.containsKey(legion.getLegionId())) {
				legion.setLegionRank(legionRanking.get(legion.getLegionId()));
				PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x01, legion));
			}
			legionsUpdated++;
		}
	...
	public void LegionWhUpdate(Player player) {
		...
		try {
			 ...
			 * 1. save items first
			 */
			InventoryDAO.store(allItems, player.getObjectId(), player.getPlayerAccount().getId(), legion.getLegionId());
	...
	public void setContributionPoints(Legion legion, long newPoints, boolean save) {
		...
		PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x03, legion));
		if (save)
			storeLegion(legion);
	}
		...
		if (legionRestrictions.canUploadEmblem(activePlayer)) {
			...
			if (legionEmblem.getUploadSize() == legionEmblem.getUploadedSize()) {
				if (legionEmblem.getUploadedSize() == 0 || legionEmblem.getUploadSize() == 0) {
					PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_WARN_CORRUPT_EMBLEM_FILE);
					return;
				} if (!activePlayer.getInventory().tryDecreaseKinah(800000)) {
					PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_MSG_NOT_ENOUGH_MONEY);
					return;
				}
				...
				LegionEmblem emblem = LegionDAO.loadLegionEmblem(activePlayer.getLegion().getLegionId());
	...
	public void sendEmblemData(Player player, LegionEmblem legionEmblem, int legionId, String legionName) {
		PacketSendUtility.sendPacket(player, new S_GUILD_EMBLEM_IMG_BEGIN(legionId, legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b(), legionName, legionEmblem.getEmblemType(), legionEmblem.getCustomEmblemData().length));
		...
		do {
			...
			if (currentSize >= maxSize) {
				...
				PacketSendUtility.sendPacket(player, new S_GUILD_EMBLEM_IMG_DATA(maxSize, bytes));
			} else {
				...
				log.debug("legionEmblem data send size: " + (bytes.length) + " bytes");
				PacketSendUtility.sendPacket(player, new S_GUILD_EMBLEM_IMG_DATA(currentSize, bytes));
			}
		}
	...
	public void setLegionName(Legion legion, String newLegionName, boolean save) {
		...
		PacketSendUtility.broadcastPacketToLegion(legion, new S_GUILD_INFO(legion));
		for (Player legionMember : legion.getOnlineLegionMembers()) {
			PacketSendUtility.broadcastPacket(legionMember, new S_CHANGE_GUILD_OTHER(legionMember.getObjectId(), legion.getLegionId(), legion.getLegionName(), legionMember.getLegionMember().getRank().getRankId()), true);
		}
		if (save)
			storeLegion(legion);
	}
	...
	private void changeAnnouncement(Player activePlayer, String announcement) {
		if (legionRestrictions.canChangeAnnouncement(activePlayer.getLegionMember(), announcement)) {
			...
			PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_WRITE_NOTICE_DONE);
			PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x05, (int) (System.currentTimeMillis() / 1000), announcement));
		}
	}
	...
	public void addHistory(Legion legion, String text, LegionHistoryType legionHistoryType, int tabId, String description) {
		...
		PacketSendUtility.broadcastPacketToLegion(legion, new S_GUILD_HISTORY(legion.getLegionHistoryByTabId(tabId), tabId));
	}
	...
	private void addLegionMember(Legion legion, Player player, LegionRank rank) {
		...
		PacketSendUtility.sendPacket(player, new S_GUILD_INFO(legion));
		...
		while (!splits.isLast()) {
			...
			PacketSendUtility.sendPacket(player, new S_GUILD_MEMBER_INFO(curentMembers, result, isFirst));
			isFirst = false;
		}
		PacketSendUtility.broadcastPacketToLegion(legion, new S_ADD_GUILD_MEMBER(player, false, 1300260, player.getName()), player.getObjectId());
		PacketSendUtility.sendPacket(player, new S_ADD_GUILD_MEMBER(player, false, 0, ""));
		LegionEmblem legionEmblem = legion.getLegionEmblem();
		PacketSendUtility.broadcastPacket(player, new S_GUILD_EMBLEM_UPDATED(legion.getLegionId(), legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b(), legionEmblem.getEmblemType()), true);
		PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_INFO(0x08));
		PacketSendUtility.broadcastPacket(player, new S_CHANGE_GUILD_OTHER(player.getObjectId(), legion.getLegionId(), legion.getLegionName(), player.getLegionMember().getRank().getRankId()), true);
	...
	private boolean removeLegionMember(String charName, boolean kick, String playerName) {
		...
		if (player != null) {
			PacketSendUtility.broadcastPacket(player, new S_CHANGE_GUILD_OTHER(player.getObjectId(), 0, "", 2), true);
		}
		Legion legion = legionMember.getLegion();
		/**
		 ...
		 */
		if (kick) {
			PacketSendUtility.broadcastPacketToLegion(legion, new S_DELETE_GUILD_MEMBER(1300247, legionMember.getObjectId(), playerName, legionMember.getName()));
		} else {
			PacketSendUtility.broadcastPacketToLegion(legion, new S_DELETE_GUILD_MEMBER(900699, legionMember.getObjectId(), charName));
		}
		return true;
	}
			...
			case 0x01:
				if (targetPlayer != null) {
					if (targetPlayer.getPlayerSettings().isInDeniedStatus(DeniedStatus.GUILD)) {
						PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_MSG_REJECTED_INVITE_GUILD(charName));
						return;
					}
					invitePlayerToLegion(activePlayer, targetPlayer);
				}
				else {
					PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_NO_USER_TO_INVITE);
				}
				break;
			/**
			...
			case LEGION_ACTION_KICK:
				...
				if (legionRestrictions.canKickPlayer(activePlayer, charName)) {
					if (removeLegionMember(charName, true, activePlayer.getName())) {
						...
						if (targetPlayer != null) {
							PacketSendUtility.sendPacket(targetPlayer, new S_DELETE_GUILD_MEMBER(1300246, 0, legion.getLegionName()));
							targetPlayer.resetLegionMember();
						}
					}
				}
			...
			case 0x02:
				if (legionRestrictions.canLeave(activePlayer)) {
					if (removeLegionMember(activePlayer.getName(), false, "")) {
						Legion legion = activePlayer.getLegion();
						PacketSendUtility.sendPacket(activePlayer, new S_DELETE_GUILD_MEMBER(1300241, 0, legion.getLegionName()));
	...
	public boolean removePlayerFromLegionAsItself(Player player) {
		if (removeLegionMember(player.getName(), false, "")) {
			Legion legion = player.getLegion();
			PacketSendUtility.sendPacket(player, new S_DELETE_GUILD_MEMBER(1300241, 0, legion.getLegionName()));
			player.resetLegionMember();
			return true;
		} else {
			return false;
	...
	public void onLogin(Player activePlayer) {
		Legion legion = activePlayer.getLegion();
		PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_MEMBER_INFO(activePlayer, 0, ""), activePlayer.getObjectId());
		PacketSendUtility.broadcastPacketToLegion(legion, S_MESSAGE_CODE.STR_MSG_NOTIFY_LOGIN_GUILD(activePlayer.getName()), activePlayer.getObjectId());
		PacketSendUtility.broadcastPacketToLegion(legion, new S_ADD_GUILD_MEMBER(activePlayer, true, 0, ""));
		PacketSendUtility.sendPacket(activePlayer, new S_GUILD_INFO(legion));
		...
		while (!splits.isLast()) {
			...
			PacketSendUtility.sendPacket(activePlayer, new S_GUILD_MEMBER_INFO(curentMembers, result, isFirst));
			isFirst = false;
		}
		displayLegionMessage(activePlayer, legion.getCurrentAnnouncement());
		if (legion.isDisbanding()) {
			PacketSendUtility.sendPacket(activePlayer, new S_CHANGE_GUILD_INFO(0x06, legion.getDisbandTime()));
		}
	}
	...
	public void onLogout(Player player) {
		...
		PacketSendUtility.broadcastPacketToLegion(legion, new S_CHANGE_GUILD_MEMBER_INFO(player));
		storeLegion(legion);
	 ...
	 * @author Simple
	 */
	public class LegionRestrictions {
		...
		private boolean canCreateLegion(Player activePlayer, String legionName) {
			if (!isValidName(legionName)) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CREATE_INVALID_GUILD_NAME);
				return false;
			} else if (!isFreeName(legionName)) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CREATE_SAME_GUILD_EXIST);
				return false;
			} else if (activePlayer.isLegionMember()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CREATE_ALREADY_BELONGS_TO_GUILD);
				return false;
			} else if (activePlayer.getInventory().getKinah() < LegionLevel.LEVEL1.getKinah()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CREATE_NOT_ENOUGH_MONEY);
				return false;
			}
			return true;
		}
		...
		private boolean canInvitePlayer(Player activePlayer, Player targetPlayer) {
			...
			if (activePlayer.getLifeStats().isAlreadyDead()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_CANT_INVITE_WHEN_DEAD);
				return false;
			} if (isSelf(activePlayer, targetPlayer.getObjectId())) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_CAN_NOT_INVITE_SELF);
				return false;
			} else if (targetPlayer.isLegionMember()) {
				if (legion.isMember(targetPlayer.getObjectId())) {
					PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_HE_IS_MY_GUILD_MEMBER(targetPlayer.getName()));
				} else {
					PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_INVITE_HE_IS_OTHER_GUILD_MEMBER(targetPlayer.getName()));
				}
				return false;
		...
		private boolean canKickPlayer(Player activePlayer, String charName) {
			...
			if (isSelf(activePlayer, legionMember.getObjectId())) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_BANISH_CANT_BANISH_SELF);
				return false;
			} else if (legionMember.isBrigadeGeneral()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_BANISH_CAN_BANISH_MASTER);
				return false;
			} else if (legionMember.getRank() == activePlayer.getLegionMember().getRank()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_BANISH_DONT_HAVE_RIGHT_TO_BANISH);
				return false;
		...
		private boolean canAppointBrigadeGeneral(Player activePlayer, Player targetPlayer) {
			...
			if (!isBrigadeGeneral(activePlayer)) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_MEMBER_RANK_DONT_HAVE_RIGHT);
				return false;
			} if (isSelf(activePlayer, targetPlayer.getObjectId())) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_MASTER_ERROR_SELF);
				return false;
		...
		private boolean canChangeLevel(Player activePlayer) {
			...
			if (legion.getLegionLevel() == MAX_LEGION_LEVEL) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_LEVEL_CANT_LEVEL_UP);
				return false;
			} else if (activePlayer.getInventory().getKinah() < legion.getKinahPrice()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_LEVEL_NOT_ENOUGH_MONEY);
				return false;
			} else if (!legion.hasRequiredMembers()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_LEVEL_NOT_ENOUGH_MEMBER);
				return false;
			} else if (legion.getContributionPoints() < levelContributionPrice) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_CHANGE_LEVEL_NOT_ENOUGH_POINT);
				return false;
			}
			return true;
		}
		...
		private boolean canDisbandLegion(Player activePlayer, Legion legion) {
			if (legion == null) {
				...
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_DISPERSE_ONLY_MASTER_CAN_DISPERSE);
				return false;
			} else if (legion.getLegionWarehouse().size() > 0) {
				return false;
			}  else if (legion.isDisbanding()) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_DISPERSE_ALREADY_REQUESTED);
				return false;
			} else if (legion.getLegionWarehouse().size() > 0) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_DISPERSE_CANT_DISPERSE_GUILD_STORE_ITEM_IN_WAREHOUSE);
				return false;
			}
			return true;
		}
		...
		private boolean canLeave(Player activePlayer) {
			if (isBrigadeGeneral(activePlayer)) {
				PacketSendUtility.sendPacket(activePlayer, S_MESSAGE_CODE.STR_GUILD_LEAVE_MASTER_CANT_LEAVE_BEFORE_CHANGE_MASTER);
				return false;
			}
			return true;
		}
		...
		public boolean canOpenWarehouse(Player player) {
			...
			if (legion.isDisbanding()) {
				PacketSendUtility.sendPacket(player, S_MESSAGE_CODE.STR_GUILD_WAREHOUSE_CANT_USE_WHILE_DISPERSE);
				return false;
			} else if (!LegionConfig.LEGION_WAREHOUSE) {
				return false;
			} else if (whUser != playerId && legWh.getWhUser() != 0) {
				PacketSendUtility.sendPacket(player, S_MESSAGE_CODE.STR_GUILD_WAREHOUSE_IN_USE);
				return false;
			}
	...
	public LegionContainer getAllCachedLegions() {
		return allCachedLegions;
	}

	public LegionMemberContainer getAllCachedLegionMembers() {
		return allCachedLegionMembers;
	}

	public LegionRestrictions getLegionRestrictions() {
		return legionRestrictions;
	}
