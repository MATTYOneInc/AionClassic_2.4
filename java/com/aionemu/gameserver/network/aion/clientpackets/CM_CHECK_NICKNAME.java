...
import com.aionemu.gameserver.network.aion.serverpackets.S_EXIST_RESULT;
	...
	protected void runImpl() {
		...
		if (!PlayerService.isFreeName(nick) || PlayerService.isOldName(nick)) {
			if (GSConfig.CHARACTER_CREATION_MODE == 2)
				client.sendPacket(new S_EXIST_RESULT(S_CREATE_CHARACTER.RESPONSE_NAME_RESERVED));
			else
				client.sendPacket(new S_EXIST_RESULT(S_CREATE_CHARACTER.RESPONSE_NAME_ALREADY_USED));
		}
		else if (!NameRestrictionService.isValidName(nick)) {
			client.sendPacket(new S_EXIST_RESULT(S_CREATE_CHARACTER.RESPONSE_INVALID_NAME));
		}
			...
			client.sendPacket(new S_EXIST_RESULT(0));
	}
}
