...
public class WalkManager {
	...
	private static final int WALK_RANDOM_RANGE = 3;
			...
			public void run() {
				if (npcAI.isInState(AIState.WALKING)) {
					if (distToSpawn > walkRange) {
						owner.getMoveController().moveToPoint(owner.getSpawn().getX(), owner.getSpawn().getY(), owner.getSpawn().getZ() - 0.1f);
					} else {
						...
						if (GeoDataConfig.GEO_ENABLE && GeoDataConfig.GEO_NPC_MOVE) {
							...
							Vector3f loc = GeoService.getInstance().getClosestCollision(owner, owner.getX() + nextX, owner.getY() + nextY, owner.getZ() - 0.1f, false, flags);
