...
public abstract class EffectTemplate {
	...
	public boolean calculate(Effect effect, StatEnum statEnum, SpellStatus spellStatus, SkillElement element) {
		if (effect.getSkillTemplate().isPassive()) {
			...
			return true;
		} if (this instanceof HealInstantEffect || this instanceof MPHealInstantEffect) {
			this.addSuccessEffect(effect, spellStatus);
			return true;
		} if (statEnum != null && this.isAlteredState(statEnum) && this.isImuneToAbnormal(effect, statEnum)) {
			return false;
		} if (effect.getIsForcedEffect()) {
			this.addSuccessEffect(effect, spellStatus);
			if (element != SkillElement.NONE) {
				effect.getEffected().getObserveController().checkAttackStatus(AttackStatus.RESIST);
			}
			return true;
		} if (!this.effectConditionsCheck(effect)) {
			return false;
		}
		return false;
	}
	...
	public void calculateSubEffect(Effect effect) {
		if (subEffect == null) {
			...
			return;
		} if (Rnd.get((int)100) > this.subEffect.getChance()) {
			return;
		}
		int duration = 0;
		...
		Effect newEffect = new Effect(effect.getEffector(), effect.getEffected(), template, level, duration);
		newEffect.setAccModBoost(effect.getAccModBoost());
		newEffect.initialize();
		if (duration > 0) {
            newEffect.setForcedDuration(true);
	...
	public boolean calculateEffectResistRate(Effect effect, StatEnum statEnum) {
		...
		if (penetrationStat != null) {
			...
			if (differ > 2 && differ < 8) {
				effectPower -= Math.round((float)(effectPower * (differ - 2)) / 15.0f);
			} else if (differ >= 8) {
				effectPower = (int)((float)effectPower * 0.1f);
			}
		...
		return Rnd.get((int)1000) <= effectPower;
	}
