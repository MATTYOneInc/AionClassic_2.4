...
public class Esoterrace_WindStream_3AI2 extends NpcAI2
{
	...
	private void startWindStream(final Npc npc) {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                Npc npc2 = (Npc) spawn(300250000, 206034, 392.0000f, 543.0000f, 318.0000f, (byte) 18, 0, 1);
				windStreamAnnounce(npc2, 1);
				WorldMapInstance instance = getPosition().getWorldMapInstance();
				deleteNpcs(instance.getNpcs(207036));
                spawn(206034, 392.0000f, 543.0000f, 318.0000f, (byte) 18);
                PacketSendUtility.broadcastPacket(npc2, new S_WIND_STATE_INFO(1, 300250000, 161, 1));
                if (npc2 != null) {
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                PacketSendUtility.sendPacket(player, new S_WIND_STATE_INFO(1, 300250000, 161, state));
            }
        });
    }
