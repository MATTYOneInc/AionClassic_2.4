...
public class DuxAI2 extends AggressiveNpcAI2
{
	...
	private void illusionGate() {
		...
		for (Player player: getKnownList().getKnownPlayers().values()) {
			final Npc illusionGate1 = getPosition().getWorldMapInstance().getNpc(295212);
			if (illusionGate1 == null) {
				ThreadPoolManager.getInstance().schedule(new Runnable() {
					...
					public void run() {
						...
						spawn(295212, getOwner().getX() + 8, getOwner().getY(), getOwner().getZ(), (byte) getOwner().getHeading());
					}
				}, 3000);
			}
		}
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				deleteNpcs(instance.getNpcs(295212));
			}
		}, 600000);
	}
