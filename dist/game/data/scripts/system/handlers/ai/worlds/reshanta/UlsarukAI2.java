...
public class UlsarukAI2 extends AggressiveNpcAI2
{
	...
	private void illusionGate() {
		...
		for (Player player: getKnownList().getKnownPlayers().values()) {
			final Npc illusionGate1 = getPosition().getWorldMapInstance().getNpc(296060);
			if (illusionGate1 == null) {
				ThreadPoolManager.getInstance().schedule(new Runnable() {
					...
					public void run() {
						...
						spawn(296060, getOwner().getX() + 8, getOwner().getY(), getOwner().getZ(), (byte) getOwner().getHeading());
					}
				}, 3000);
			}
		}
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				deleteNpcs(instance.getNpcs(296060));
			}
		}, 600000);
	}
