...
public class _19014Master_Armorsmiths_Potential extends QuestHandler
{
	private final static int questId = 19014;
	
	public _19014Master_Armorsmiths_Potential() {
		super(questId);
	}
	
	@Override
	public void register() {
		qe.registerOnFailCraft(182206765, questId);
	...
	public boolean onDialogEvent(final QuestEnv env) {
        ...
        if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 203791) {
				...
				long eremitiaNobleBreastplate = player.getInventory().getItemCountByItemId(182206765);
				switch (env.getDialog()) {
					case START_DIALOG: {
						if (eremitiaNobleBreastplate >= 1 && var == 1) {
							return sendQuestDialog(env, 1352);
						} else if (eremitiaNobleBreastplate == 0 && var == 1) {
