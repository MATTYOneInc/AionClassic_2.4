...
public class _29008Master_Weaponsmiths_Potential extends QuestHandler
{
	private final static int questId = 29008;
	
	public _29008Master_Weaponsmiths_Potential() {
		super(questId);
	}
	
	@Override
	public void register() {
		qe.registerOnFailCraft(182207898, questId);
	...
	public boolean onDialogEvent(final QuestEnv env) {
        ...
        if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 204105) {
				...
				long agehiaNobleGreatsword = player.getInventory().getItemCountByItemId(182207898);
				switch (env.getDialog()) {
					case START_DIALOG: {
						if (agehiaNobleGreatsword >= 1 && var == 1) {
							return sendQuestDialog(env, 1352);
						} else if (agehiaNobleGreatsword == 0 && var == 1) {
