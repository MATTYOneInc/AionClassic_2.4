...
public class _19032Master_Alchemists_Potential extends QuestHandler
{
	private final static int questId = 19032;
	
	public _19032Master_Alchemists_Potential() {
		super(questId);
	}
	
	@Override
	public void register() {
		qe.registerOnFailCraft(182206768, questId);
	...
	public boolean onDialogEvent(final QuestEnv env) {
        ...
        if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 203787) {
				...
				long eremitiaNobleSpellbook = player.getInventory().getItemCountByItemId(182206768);
				switch (env.getDialog()) {
					case START_DIALOG: {
						if (eremitiaNobleSpellbook >= 1 && var == 1) {
							return sendQuestDialog(env, 1352);
						} else if (eremitiaNobleSpellbook == 0 && var == 1) {
