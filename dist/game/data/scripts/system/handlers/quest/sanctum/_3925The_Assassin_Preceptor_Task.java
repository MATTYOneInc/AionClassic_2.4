...
public class _3925The_Assassin_Preceptor_Task extends QuestHandler
{
	private final static int questId = 3925;
	
	public _3925The_Assassin_Preceptor_Task() {
		super(questId);
	}
	
	@Override
	public void register() {
		qe.registerQuestSkill(851, questId);
		qe.registerQuestSkill(858, questId);
	...
	public boolean onUseSkillEvent(QuestEnv env, int skillUsedId) {
		...
		if (qs != null && qs.getStatus() == QuestStatus.START) {
			if (skillUsedId == 851 || skillUsedId == 858) {
				int var = qs.getQuestVarById(0);
				if (var >= 0 && var < 9) {
