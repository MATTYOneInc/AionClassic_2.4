	...
	public void execute(final Player admin, String... params) {
		if (admin.getAccessLevel() < 5) {
			...
			if (admin.getTarget() instanceof Npc) {
                ...
                PacketSendUtility.sendMessage(admin, "SPAWN ID: " + spawnId);
                if (spawnId != null) {
                    ...
                    PacketSendUtility.sendMessage(admin, comment.toString() + " ");
                }
			} else {
            ...
            for (final SpawnTemplate template : templates) {
                ...
                task = ThreadPoolManager.getInstance().schedule(new Runnable() {
                    ...
                    public void run() {
                        if (npc != null) {
                            PacketSendUtility.broadcastPacketAndReceive(admin2, new S_MOVEBACK(npc, admin2));
                        }
                    }
                }, time);
                time += 3000;
                task = ThreadPoolManager.getInstance().schedule(new Runnable() {
                    ...
                    public void run() {
                        if (npc != null) {
                            ...
                            ++spawned;
                            PacketSendUtility.sendMessage(admin2, spawned + ". " + comment.toString() + " ");
                            npc = null;
                        } else {
                            if (template != null) {
            ...
            PacketSendUtility.sendMessage(admin, "Syntax: //fixnpc <start>");
        }
