		...
		if (params[0].equalsIgnoreCase("set")) {
			...
			if (params[1].equalsIgnoreCase("w")) {
				...
				if (walkerId != null) {
					...
					String finalWalkerId2 = walkerId;
					List<SpawnTemplate> sameIds = allSpawns.stream().flatMap(o -> o.getSpawnTemplates().stream().filter(w -> w.getWalkerId().equals(finalWalkerId2))).collect(Collectors.toList());
